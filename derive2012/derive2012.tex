...
Therefore, we reconcile the returned set of events based on the simple, yet effective Levenshtein distance on the event titles with an empirically determined editing distance of five. In consequence, we are capable to detect that event names like \emph{e.g.}, \emph{Titanic} and \emph{Titanic 3D} correspond to the same movie event.
