...
of the 500 million objects~\cite{singhal2012_1} in the graph, and
...
We have modeled the plaintext graph terms (or predicates)
...
Similar to the unknown list of objects in the graph (see \autoref{sec:on-crowdsourcing}),
there is no known list of graph terms,
which makes a~complete mapping impossible.
We have collected \todo{Add Number of Collected Graph Terms} graph terms
as of time of writing, however, the mapping to Linked Data terms
will be a~permanent work in progress.
\autoref{code:json} shows a~subset of Chuck Norris' Knowledge Graph data in plain JSON,
\autoref{code:jsonld} the lifted, meaningful JSON-LD version.

\begin{lstlisting}[caption=Subset of the extracted plain JSON from the Chuck Norris Knowledge Graph data., label=code:json, float=h, escapechar=§]
{
  "@id": "http://openknowledgegraph.org/data/H4sIAAAAA[...]",
  "Topic_Of": "http://en.wikipedia.org/wiki/Chuck_Norris",
  "Name": "Chuck Norris",
  "Fact": [
    "Chuck Norris can cut thru a knife w/ butter."
  ],
  "Full_name": [
    "Carlos Ray Norris"
  ],
  ...
  "Spouse": [
    ...
    },
    {
      ...
      "text": "Dianne Holechek"
    }
  ]
}
\end{lstlisting} 

\begin{lstlisting}[caption=Subset of the lifted and meaningful JSON-LD from the Chuck Norris Knowledge Graph data. The mapping of the graph terms can be seen in the @context., label=code:jsonld, float=h, escapechar=§]
{
  "@id": "http://openknowledgegraph.org/data/H4sIAAAAA[...]",
  "@context": {
